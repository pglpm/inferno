set.seed(17)
mypdf('frequencies_dataset_custom')
ndata <- 1e4
...
Cdata[Cdata <= 0] <- 0
Cdata[Cdata >= 5] <- 5
myhist(Cdata, plot=T, border.alpha=1, xlab='Cvrt', ylab=NA)

## Discrete variate
clus <- sample(1:2, ndata, prob=c(0.75, 0.25), replace=TRUE)
Ddata <- round(rnorm(n=ndata, mean=means[clus], sd=sds[clus])*5)/5
myhist(Ddata, plot=T, border.alpha=1, xlab='Dvrt', ylab=NA)

## Discrete bounded variate
clus <- sample(1:2, ndata, prob=c(0.75, 0.25), replace=TRUE)
## Dudata <- round(plogis(rnorm(n=ndata, mean=means[clus], sd=sds[clus]))*10)/10
Dudata <- round(Cdata*5)/5
myhist(Dudata, plot=T, border.alpha=1, xlab='Duvrt', ylab=NA)

## Binary variate
Bdata <- sample(c('no','yes'), ndata, replace=T, prob=c(0.25, 0.75))
myflexiplot(x=c('no','yes'), y=c(0.25,0.75), type='b', ylim=c(0,1), xlab='Bvrt', ylab=NA)
...
myhist(Ndata, plot=T, border.alpha=1, xlab='Nvrt', ylab=NA)
...
myhist(Odata, plot=T, xlab='Ovrt', ylab=NA)
...
myhist(N2data, plot=T, border.alpha=1, xlab='N2vrt', ylab=NA)
dev.off()
