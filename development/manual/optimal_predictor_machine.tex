...
%% Last-Updated: 2025-06-05T23:54:40+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
In order to specify the probability densities $\p(\di F \| I)$ and $\p(\di F \| D, I)$ we need a mathematical representation of the possible joint distributions $F(y,x)$ and of their space. From a differential-geometric point of view \autocites[III.A, VII.A]{choquetbruhatetal1977_r1996}, we want a coordinate system on the manifold of distributions $\set{F}$. It would be an advantage if this mathematical representation also let us easily compute conditional and marginal distributions such as $F(y\| x)$ and $F(x)$, because we need these to calculate our main probabilities from formulae~\eqref{eq:general_prob_partialexch} or \eqref{eq:indepYfromdata}.
...
\item If the number of values is larger than 10, then the ordinal variate is mathematically treated as an interval-discrete one (\sect~\ref{sec:variates_rounded}).
\end{itemize}
...
\subsection{Explicit form of the density $\p(\di F\|D,I)$}
...
\begin{acknowledgements}
  \ldots to Mari \amp\ Miri for continuous encouragement and affection, and to Buster Keaton and Saitama for filling life with awe and inspiration. To the developers and maintainers of \LaTeX, Emacs, AUC\TeX, Open Science Framework, R, Octave, Python, Inkscape, LibreOffice, Sci-Hub, Libgen for making a free and impartial scientific exchange possible.
