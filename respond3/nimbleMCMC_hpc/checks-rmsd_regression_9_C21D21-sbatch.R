...
## Last-Updated: 2021-10-21T06:22:34+0200
################
...
seed <- 149
baseversion <- 'CHECK2regressionC21D21b_'
nclusters <- 2L^6
...
if(file.exists(parmListTestFile)){
    ...
    parmListTest <- list()
    ##
    parmListTest$q <- matrix(rdirch(n=1, alpha=rep(2/nclusters, nclusters)), nrow=1)
    ##
...
list(
    ...
    ##
    ## Check 1: shapes were 1 instead of 1/2, probshapes 32 instead of 16
    ## Check 2
    ...
    ## C=rep(1, ndata)
    ## Test 5 old 
    ## meanC=matrix(rnorm(n=nccovs*(nclusters), mean=medianccovs, sd=widthccovs), nrow=nccovs, ncol=nclusters),
    ## tauC=matrix(rgamma(n=nccovs*(nclusters), shape=1, rate=widthccovs^2), nrow=nccovs, ncol=nclusters),
    ## probD=matrix(rbeta(n=ndcovs*(nclusters), shape1=1, shape2=1), nrow=ndcovs, ncol=nclusters),
    ## sizeD=apply(matrix(rnbinom(n=ndcovs*(nclusters), prob=1/2, size=maxdcovs), nrow=ndcovs, ncol=nclusters), 2, function(x){maxdcovs*(x<maxdcovs)+x*(x>=maxdcovs)}),
    ## C=rcat(n=ndata, prob=rep(1/nclusters,nclusters))
##
    ## meanC=matrix(rnorm(n=nccovs*(nclusters), mean=medianccovs, sd=1/sqrt(rgamma(n=nccovs, shape=1/2, rate=rgamma(n=nccovs, shape=1/2, rate=1/(widthccovs/2)^2)))), nrow=nccovs, ncol=nclusters),
    ## tauC=matrix(rgamma(n=nccovs*(nclusters), shape=1/2, rate=rgamma(n=nccovs, shape=1/2, rate=1/(widthccovs/2)^2)), nrow=nccovs, ncol=nclusters),
...
for(stage in 0:nstages){
    ...
    miqrtracesTest <- calcSampleMQ(parmListTest)
    probCheckpointsTest <- t(probValuesSamples(checkpoints, parmListTest))
    mediansTest <- matrix(miqrtracesTest[,,1], nrow=1)
    colnames(mediansTest) <- paste0('MEDIAN_',colnames(miqrtracesTest))
    Q1sTest <- matrix(miqrtracesTest[,,2], nrow=1)
    colnames(Q1sTest) <- paste0('Q1_',colnames(miqrtracesTest))
    Q3sTest <- matrix(miqrtracesTest[,,3], nrow=1)
    colnames(Q3sTest) <- paste0('Q3_',colnames(miqrtracesTest))
    iqrsTest <- Q3sTest - Q1sTest
    colnames(iqrsTest) <- paste0('IQR_',colnames(miqrtracesTest))
    tracesTest <- cbind(probCheckpointsTest, mediansTest, iqrsTest, Q1sTest, Q3sTest, do.call(cbind, momentstracesTest))
...
##
    for(acov in colnames(traces)){
        ...
        matplot(transf(traces[,acov]), type='l', lty=1, col=colpalette[acov],
                ...
                ylab=acov,
                ylim=range(c(transf(rbind(traces,tracesTest)[,acov][abs(transf(rbind(traces,tracesTest)[,acov]))<Inf]))))
