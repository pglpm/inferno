...
## Last-Updated: 2021-10-09T14:01:58+0200
################
...
seed <- 149
baseversion <- 'checksHMUUU'
nclusters <- 2L^3 # 2L^6
ndata <- 2L^6 # nSamples = 37969
niter <- 2L^11
nstages <- 10L
ncheckpoints <- 8L
covNames <-  c('log_RMSD'
...
list(
    ...
    sizeD=matrix(rnbinom(n=ndcovs*nclusters, prob=sizeQdcovs[1,], size=sizeQdcovs[2,]), nrow=ndcovs, ncol=nclusters) + maxdcovs,
...
for(stage in 0:nstages){
    ...
    grouplegends <- foreach(agroup=1:length(tracegroups))%do%{
        c( paste0('-- STATS ', names(tracegroups)[agroup], ' --'),
          ...
          paste0('rel.err.: ', max(diagnSE[tracegroups[[agroup]]])))
    }
    ##
    ## plan(sequential)
