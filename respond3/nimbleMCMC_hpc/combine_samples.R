...
## Last-Updated: 2021-10-10T08:22:57+0200
################
...
##
gc()
parmListTest <- readRDS(file=paste0('_parmListTest-R',baseversion,'-V',length(covNames),'-K',nclusters,'.rds'))
...
for(stage in 1:nstages){
    ...
    ##
    mcsamples <- rbind(mcsamples,
                       readRDS(file=paste0('_mcsamples-R',version,'-V',length(covNames),'-D',ndata,'-K',nclusters,'-I',niter,'.rds')) )
}
mcsamples <- mcsamples[seq(1,nrow(mcsamples),length.out=2048),]
    ##
    ...
    ##
    pdff(paste0('mcsummary-comboR',version,'-V',length(covNames),'-D',ndata,'-K',nclusters,'-I',nrow(mcsamples)))
