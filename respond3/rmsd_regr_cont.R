...
## #
## system.time(testmc <- profRegr(excludeY=FALSE, yModel='Categorical', xModel='Mixed', nSweeps=500, nBurn=100, nFilter=5, data=datamc, nClusInit=10, outcome='bin_RMSD', covNames=c(discreteCovs,continuousCovs), discreteCovs=discreteCovs, continuousCovs=continuousCovs, nProgress=100, seed=222, output='-mcoutput'))
## #
## pdff('cmsum_y_discr')
...
for(i in discreteCovs){
    ...
    missing <- which(tabulate(datum-min(datum)+1)==0)
    ##print(missing)
    for(j in missing-1+min(datum)){
            dt <- data.table(j)
            names(dt) <- i
...
system.time(testres <- t(apply(testdata, 1, function(datum){
    c(datum['bin_RMSD']-discrMin['bin_RMSD'], colMeans(predictYpar(testmc,datum)))
})))
##
