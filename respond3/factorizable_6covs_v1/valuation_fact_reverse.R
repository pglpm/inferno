...
##################################################################
##################################################################
## Function to calculate \sum_t F^{(t)}_{x|r}
##
predictYX <- function(dataobj, X){
    dC <- setdiff(discreteCovs,'bin_RMSD')
    X <- as.matrix(X[, c(dC,continuousCovs), with=FALSE])
    freqs <- foreach(val=rmsdVals, .combine=rbind)%:%foreach(asample=seq_along(dataobj[[val]]$nList), .combine=cbind, .inorder=FALSE)%dopar%{
        ##
        colSums(exp( log(normalize(dataobj[[val]]$psiList[[asample]])) +
...
for(dset in c('tail', 'head')){
    ...
    condfreqs <- predictYX(MCMCdata, testdata)
        plan(sequential)
        condfreqs[,,1] <- normalizem(t(t(condfreqs[,,1]) * priorR))
    ##
    set.seed(247)
    scores[[dset]][[rownames(basepriors)[ii]]] <- metrics(truevalues=testdata[,bin_RMSD], probX=condfreqs[,,1], chanceprior=priorR)
    print(dset)
    pdff(paste0('scores_reverse_fact_P',rownames(basepriors)[ii],'_',dset))
...
save(list=c('scores'), file=paste0('scores_reverse_fact_T',nTest*3,'_N',ndata,'_',length(covNums),'covs.RData'))
