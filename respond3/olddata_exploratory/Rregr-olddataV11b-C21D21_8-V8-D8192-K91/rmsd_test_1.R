...
## Last-Updated: 2021-11-02T08:21:47+0100
################
...
seed <- 149
baseversion <- 'regr-olddataV11b-C21D21_'
...
for(stage in 0:nstages){
    ...
    stage <- 8
    version <- paste0(baseversion, stage)
    mcsamples <- readRDS(paste0('_mcsamples-R',version,'-V',length(covNames),'-D',ndata,'-K',nclusters,'-I',niter,'.rds'))
    ...
    ##
samplesvar <- foreach(asample=seq_len(nrow(q)), .combine=c, .inorder=FALSE)%dopar%{
        ...
        ##
        sum(colSums(
        exp(2*(parmList$meanC[asample,'log_RMSD',]+1/parmList$tauC[asample,'log_RMSD',])) * W)/colSums(W) -
            (colSums(
                     exp(parmList$meanC[asample,'log_RMSD',]+1/(2*parmList$tauC[asample,'log_RMSD',])) * W)/colSums(W))^2)/ndataz
    }


samplesvar2 <- foreach(asample=seq_len(nrow(q)), .combine=c, .inorder=FALSE)%dopar%{
        ## W: rows=clusters, cols=datapoints
        W <- exp(
            log(q[asample,]) +
            t(vapply(seq_len(ncol(q)), function(acluster){
    ...
    mean(
        (denom * colSums(
        exp(2*(parmList$meanC[asample,'log_RMSD',]+1/parmList$tauC[asample,'log_RMSD',])) * W) -
            colSums(
                     exp(parmList$meanC[asample,'log_RMSD',]+1/(2*parmList$tauC[asample,'log_RMSD',])) * W)^2)/denom^2, na.rm=T)
    }


sampleslogvar <- foreach(asample=seq_len(nrow(q)), .combine=c, .inorder=FALSE)%dopar%{
        ## W: rows=clusters, cols=datapoints
        W <- exp(
            log(q[asample,]) +
            t(vapply(seq_len(ncol(q)), function(acluster){
    ...
    mean(
        (denom * colSums(
        (1/parmList$tauC[asample,'log_RMSD',]+parmList$meanC[asample,'log_RMSD',]^2) * W) -
            colSums(parmList$meanC[asample,'log_RMSD',] * W)^2)/denom^2, na.rm=T)
    }



    
    foreach(asample=seq_len(nrow(q)), .combine=c, .inorder=FALSE)%:%foreach(adatum=seq_len(nrow(alldata)), .combine='+', .inorder=FALSE)%dopar%{
        ## W: rows=clusters, cols=datapoints
        W <- exp(
            log(q[asample,]) +
            t(vapply(seq_len(ncol(q)), function(acluster){
        ...
        exp(2*(parmList$meanC[asample,'log_RMSD',]+1/parmList$tauC[asample,'log_RMSD',])) * W)/colSums(W) -
            (colSums(
                     exp(parmList$meanC[asample,'log_RMSD',]+1/(2*parmList$tauC[asample,'log_RMSD',])) * W)/colSums(W))^2
    }/ndataz
    ...
    pdff('noise_study')
##
    sdsrmsd <- sqrt(samplesvar2)
    sdsrmsd <- sdsrmsd[(!is.na(sdsrmsd)) & (sdsrmsd<+Inf)]
##
    sdslogrmsd <- sqrt(sampleslogvar)
  ##  
    hist(sqrt(sdsrmsd)[sqrt(sdsrmsd)<8],breaks=seq(0,8,by=0.125),freq=F,
         main='probability distribution of error (truncated on the right)\n conditional on first 7 features',  xlab='SD of RMSD')
    grid(nx = 8*4, ny = NULL, lty = 2, col = "gray", lwd = 1)
    legend('top',paste0('mean = infinity, median = ',signif(median(sdsrmsd),2)),bty='n')
##
    hist(sqrt(sdslogrmsd),freq=F,breaks=64,
         main='probability distribution of error\n conditional on first 7 features', xlab='SD of log(RMSD)')
    grid(nx = NULL, ny = NULL, lty = 2, col = "gray", lwd = 1)
    legend('top',paste0('mean = ',signif(mean(sdslogrmsd),2),', median = ',signif(median(sdslogrmsd),2)),bty='n')
    dev.off()
