...
finitemix <- nimbleCode({
    ...
    walpha0[1:nclusters] <- probalpha0[1:nclusters] * 2^Alpha
...
todelete <- Cmcsampler$run(niter=15000, thin=150, thin2=1, nburnin=0, time=TRUE, reset=FALSE, resetMV=FALSE)
thistime <- Sys.time()-thistime
print(thistime)
mcsamples <- as.matrix(Cmcsampler$mvSamples)
tplot(y=(mcsamples[,'Alpha']))
...
##
pdff(paste0('xprior2D_Mvar_S2shapes_A',minalpha,'_',maxalpha,'_S',minshape,'_',maxshape),apaper=3)
