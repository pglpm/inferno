...
initsFunction <- function(){
    ...
    Rrate <- matrix(rinvgamma(n=nvar*nclusters, shape=shape0[Shapeindexhi], rate=Rvar1), nrow=nvar, ncol=nclusters)
    Rvar <- matrix(rinvgamma(n=nvar*nclusters, shape=shape0[Shapeindexlo], rate=Rrate), nrow=nvar, ncol=nclusters)
...
finitemix <- nimbleCode({
    ...
    for(k in 1:nclusters){
        for(v in 1:nvar){
            ...
            Rrate[v, k] ~ dinvgamma(shape=shape0[Shapeindexhi[v]], rate=Rvar1[v])
            Rvar[v, k] ~ dinvgamma(shape=shape0[Shapeindexlo[v]], rate=Rrate[v, k])
        }
    }
    for(d in 1:npoints){
...
todelete <- Cmcsampler$run(niter=16000, thin=1, thin2=1, nburnin=15000, time=TRUE, reset=TRUE, resetMV=TRUE)
...
print('shapehi')
table(shape0[mcsamples[,'Shapeindexhi[1]']])
table(shape0[mcsamples[,'Shapeindexhi[2]']])
mean(log2(shape0[mcsamples[,extract('Shapeindexhi')]]))
print('shapelo')
table(shape0[mcsamples[,'Shapeindexlo[1]']])
table(shape0[mcsamples[,'Shapeindexlo[2]']])
mean(log2(shape0[mcsamples[,extract('Shapeindexlo')]]))
...
##
pdff(paste0('testmchyper-mean_var-sd_2shapesrate'))
for(i in 1000:990){
