...
tplot(x=mtestp[1,],y=mtestp[2,],type='p',pch='.',
      xlim=c(-1,1)*max(mtestp),
      ylim=c(-1,1)*max(mtestp)
      )
...
initsFunction <- function(){
    ...
    Rrate <- matrix(rinvgamma(n=nvar*nclusters, shape=shapehi0, rate=1), nrow=nvar, ncol=nclusters)
...
finitemix <- nimbleCode({
    ...
    for(k in 1:nclusters){
        for(v in 1:nvar){
            ...
            Rrate[v, k] ~ dinvgamma(shape=shapehi0, rate=1)
...
todelete <- Cmcsampler$run(niter=2000, thin=1, thin2=1, nburnin=1000, time=FALSE, reset=TRUE, resetMV=TRUE)
