...
predictRmeanX <- function(dataobj, X){
    ...
    freqs <- foreach(asample=seq_along(dataobj$nList), .combine=cbind, .inorder=FALSE)%dopar%{
        ...
        conds <- t(vapply(seq_len(dataobj$nList[asample]), function(cluster){
            ...
            ##
            dataobj$muList[[asample]]['log_RMSD',cluster] +
...
predictfX <- function(dataobj, X){
    ...
    freqs <- foreach(asample=seq_along(dataobj$nList), .combine=cbind, .inorder=FALSE)%dopar%{
        ...
        conds <- t(vapply(seq_len(dataobj$nList[asample]), function(cluster){
            invM <- dataobj$sigmaList[[asample]]['log_RMSD',cC,cluster] %*% solve(as.matrix(dataobj$sigmaList[[asample]][cC,cC,cluster]))
            ##
            dnorm(x=rep(rgrid, each=ntestdata),
                  mean=dataobj$muList[[asample]]['log_RMSD',cluster] +
                      invM %*% (t(XX[,cC]) - dataobj$muList[[asample]][cC,cluster]),
                  sd=sqrt(
...
condfreqs <- predictRmeanX(MCMCdata[[case]], testdata)
plan(sequential)
##
gains <- lossfunction(testdata$log_RMSD, condfreqs[,1])
chancegains <- lossfunction(testdata$log_RMSD, log(2.5))
##
basescores <- data.table(model=gains, chance=chancegains)
##
pdff(paste0('quadraticscores_direct_nonfact_conditional_P',rownames(basepriors)[ii],'_',dset))
##
print(
