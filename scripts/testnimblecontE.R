...
finitemix <- nimbleCode({
    ...
    for(k in 1:nclusters){
        for(v in 1:nvar){
            ...
            Rrate[v, k] ~ dinvgamma(shape=shape0[Shapeindexhi[v]], rate=1)
...
confnimble <- configureMCMC(Cfinitemixnimble,
                            monitors=c('Alphaindex', 'K',  'W', 'Rmean', 'Rvar', 'Rrate', 'Shapeindexhi', 'Shapeindexlo')
                            )
print(confnimble)
...
##
pdff(paste0('testmchyper_fullnovar'))
for(i in 1000:990){
