...
## Last-Updated: 2022-12-27T20:30:53+0100
#########################################
...
baseversion <- '_inferencep4prior' # Base name of output directory
nchains <- 64L 
samplesperchain <- 16L # 64 * 4 with 16 parallel runs gives 4096 samples
ncores <- 1
...
posterior <- FALSE # if set to FALSE it samples and plots prior samples
...
tmcsamples <- readRDS('_inferencep4prior/_jointmcsamples--R_inferencep4prior-V13-D704-K64-I1024---4019.rds')
vari <- 'AVDEL30MIN_neuro'
xgrid <- cbind(varinfo$min[vari]:varinfo$max[vari])
colnames(xgrid) <- vari
ygrid <- samplesFDistribution(Y=xgrid, mcsamples=tmcsamples, varinfo=varinfo, jacobian=TRUE)
mygrid <- rowMeans(ygrid)
nsamps <- 16
susq <- round(seq(1,nrow(tmcsamples),length.out=nsamps^2))
pdff(paste0('priorexamples_',vari))
par(mfrow=c(nsamps,nsamps),mar = c(0,0,0,0))
for(i in 1:(nsamps^2-1)){
    tplot(x=xgrid, y=ygrid[,susq[i]],
