...
## Last-Updated: 2021-09-15T14:45:26+0200
################
...
names(breakFeatures) <- names(alldata)
##
##
...
if(doplots==TRUE){
    ...
    for(i in 1:ncol(alldata)){
        ...
        print(ggplot(alldata[,..i], aes_(x=as.name(names(alldata)[i]))) + geom_histogram(breaks=breaks))
    }
    dev.off()
    ##
...
##
fwrite(alldata,'data_processed_transformed_rescaled.csv', sep=' ')
## Shuffle the data for training and test
set.seed(222)
alldata <- alldata[sample(1:nrow(alldata))]
fwrite(alldata,'data_processed_transformed_rescaled_shuffled.csv', sep=' ')
