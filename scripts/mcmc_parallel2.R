...
## Last-Updated: 2022-12-25T19:56:25+0100
#########################################
...
baseversion <- '_testparallel2' # *** ## Base name of output directory
maxstages <- 64
ncores <- 1
...
nsamples <- maxstages #8*ceiling((requiredESS*1.5)/8) # number of samples AFTER thinning
...
while(continue){
    ...
    gc()
        ...
        newmcsamples <- Cmcsampler$run(niter=niter+1, thin=thin, thin2=thin2, nburnin=1, time=showsamplertimes, reset=TRUE, resetMV=TRUE)
        if(showsamplertimes){
...
##        newmcsamples <- Cmcsampler$run(niter=1024, thin=1, thin2=1, nburnin=0, time=T)
    ...
    if(posterior){
        ...
        ##
        ########################################################
        #### CHECK IF AUTOCORRELATION IS SHORTER THAN CHAIN ####
        ########################################################
        if(max(diagnIAT) < nrow(newmcsamples)){
            print('Autocorrelation shorter than sample length. Keeping final sample')
            mcsamples <- rbind(mcsamples,newmcsamples[nrow(newmcsamples),,drop=F])
            finaltraces <- rbind(finaltraces,traces[nrow(traces),,drop=F])
            stage <- stage + 1
            if(savetempsamples || stage >= maxstages){
                saveRDS(mcsamples,file=paste0(dirname,'_mcsamples-R',basename,'--',mcmcseed,'-',stage,'.rds'))
            }
        }else{
            print('Autocorrelation longer than sample length. Discarding final sample')
            niter <- round(max(diagnIAT))*2
        }
    ...
    ## Summary stats
        ...
        legend(x='center', bty='n', cex=1,
               legend=c(
                   ...
                   'NOTES:',
                   if(any(is.na(newmcsamples))){'some NA MC outputs'},
                   if(any(!is.finite(newmcsamples))){'some infinite MC outputs'},
...
dev.off()
    ...
    if(plottempdistributions | !continue){
        ...
        nfsamples <- min(nfsamples, nrow(newmcsamples))
        ...
        for(v in unlist(variate)){#cat(avar)
            ...
            colnames(Xgrid) <- v
            plotsamples <- samplesFDistribution(Y=Xgrid, mcsamples=newmcsamples, varinfo=varinfo, subsamples=round(seq(1,nrow(newmcsamples),length.out=nfsamples)), jacobian=TRUE)
            ##
            if(posterior){
                par(mfrow=c(1,1))
...
saveRDS(finaltraces,file=paste0(dirname,'_mctraces-R',basename,'--',mcmcseed,'-','F','.rds') )
...
pdff(paste0(dirname,'mcmcplottraces-R',basename,'--',mcmcseed,'-','F'),'a4')
## Summary stats
matplot(1:2, type='l', col='white', main=paste0('Stats stage ',stage), axes=FALSE, ann=FALSE)
legendpositions <- c('topleft','topright','bottomleft','bottomright')
        for(alegend in 1:length(grouplegends)){
