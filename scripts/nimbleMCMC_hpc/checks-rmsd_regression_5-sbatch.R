...
## Last-Updated: 2021-10-09T08:51:37+0200
################
...
seed <- 149
baseversion <- 'checksHM'
nclusters <- 2L^4 # 2L^6
ndataOrig <- 2L^12 # nSamples = 37969
ndata <- 2L^9 # nSamples = 37969
niter <- 2L^11
nstages <- 10L
ncheckpoints <- 8L
covNames <-  c('log_RMSD'
...
## set.seed(seed)
## qTest <- rdirch(n=1, alpha=inits$alphaK)
## dim(qTest) <- c(1, length(qTest))
## meanCTest <- array(rnorm(n=nccovs*nclusters, mean=inits$meanCmean, sd=1/sqrt(inits$meanCtau)), dim=c(1, nccovs, nclusters))
## tauCTest <- array(rgamma(n=nccovs*nclusters, shape=inits$tauCshape, rate=inits$tauCrate), dim=c(1, nccovs, nclusters))
## dimnames(meanCTest) <- dimnames(tauCTest) <- list(NULL, continuousCovs, NULL)
## ##
## probDTest <- array(rbeta(n=ndcovs*nclusters, shape1=inits$probDshape1, shape2=inits$probDshape2), dim=c(1, ndcovs, nclusters))
## sizeDTest <- array(rnbinom(n=ndcovs*nclusters, prob=inits$sizeDpar1, size=inits$sizeDpar2), dim=c(1, ndcovs, nclusters))
## dimnames(probDTest) <- dimnames(sizeDTest) <- list(NULL, discreteCovs, NULL)
##
##
...
parmListTest <- mcsamples2parmlist(readRDS('_mcsamples-RpostHM3-V8-D4096-K64-I2048.rds'))
noldsamples <- nrow(parmListTest$q)
Kchoose <- sample(1:ncol(parmListTest$q), nclusters)
parmListTest$q <- parmListTest$q[noldsamples,Kchoose,drop=FALSE]
parmListTest$q <- parmListTest$q/sum(parmListTest$q)
for(aparm in 2:length(parmListTest)){
    parmListTest[[aparm]] <- parmListTest[[aparm]][noldsamples,,Kchoose,drop=FALSE]
}
...
tauQccovs <- sapply(continuousCovs, function(acov){
    fn <- function(parms){
        (pinvgamma(varsccovs[acov]/sqrt(10), shape=exp(parms[1]), scale=exp(parms[2])) - 0.005)^2 +
            (pinvgamma(varsccovs[acov]*sqrt(10), shape=exp(parms[1]), scale=exp(parms[2])) - 0.995)^2
    }
...
sizeQdcovs <- sapply(discreteCovs, function(acov){
    fn <- function(parms){
        (pnbinom(round(maxdcovs[acov]/sqrt(10)), prob=plogis(parms[1]), size=exp(parms[2])) - 0.005)^2 +
            (pnbinom(round(maxdcovs[acov]*sqrt(10)), prob=plogis(parms[1]), size=exp(parms[2])) - 0.995)^2
    }
    out <- myoptim(c(0.5, 1), fn=fn)$par
...
for(stage in 0:nstages){
    ...
    diagnSE <- apply(traces, 2, function(x){funMCSE(x)})
    ##
    ...
    colpalette <- sapply(colnames(traces),function(acov){
        ...
        else if(grepl('^logabsDcov', acov)){2}
    ...
    for(acov in colnames(traces)){
        matplot(traces[, acov], type='l', lty=1, col=colpalette[acov],
                main=paste0(acov,
                            ...
                            ' | stat: ', diagnStat[acov],
                            ' | burn: ', diagnBurn[acov],
                            ' | rel.err.true: ', signif((mean(traces[,acov])-tracesTest[1,acov])/diagnSE[acov], 3)
                            ),
                ylab=acov,
                ylim=range(c(traces[,acov], tracesTest[,acov][abs(tracesTest[,acov])<Inf])))
        matlines(x=c(1,nrow(traces)), rep(tracesTest[,acov],2), lwd=5, lty=1, col=paste0('#000000','88'))
    }
    dev.off()
