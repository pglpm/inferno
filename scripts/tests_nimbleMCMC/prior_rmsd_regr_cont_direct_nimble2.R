...
## Last-Updated: 2021-09-15T12:11:55+0200
################
...
xlim <- c(-1,1) * max(abs(plotvarRanges[,rownames(xsamples)[1]]),10)
ylim <- c(-1,1) * max(abs(plotvarRanges[,rownames(xsamples)[2]]),10)
##
...
for(asample in subsamplep){
    matplot(x=xsamples[1,subsamplex,asample],y=xsamples[2,subsamplex,asample], type='p', pch=1, lwd=1, xlab=rownames(xsamples)[1], ylab=rownames(xsamples)[2], xlim=xlim, ylim=ylim, asp=1)
