...
allmcmcrun <- foreach(case=cases, .inorder=FALSE)%dopar%{
    ...
    setwd(outdir)
    ##
    seldata <- if(ndata >= 1){1:ndata}else{0}
    datamc <- alldata[seldata, covNames, with=F]
    ## make sure all levels of discrete variables appear
    for(covariate in discreteCovs){
        datum <- datamc[[covariate]]
        datum <- datum[!is.na(datum)]
        levels <- as.numeric(names(table(alldata[[covariate]])))
        for(level in setdiff(min(levels):max(levels), as.numeric(names(table(datum))))){ #print(paste0(case,' ',covariate,' ',level))
            dt <- data.table(level)
            names(dt) <- covariate
            datamc <- rbind(datamc, dt, fill=TRUE)
        }
    }
    saveRDS(datamc, file=paste0('_datamc','.rds'))
##
    ##
    sink(file='_progress.txt')
    runtime <- Sys.time()
    mcmcrun <- c(profRegr(excludeY=TRUE, xModel='Mixed',
                          nSweeps=2000e1,
                          nBurn=3000e1,
                          nFilter=2e1,
                          nProgress=100e1,
    ...
    saveRDS(mcmcrun, file=paste0('_mcmcrun','.rds'))
    sink()
    ##
    ## Save output from files into a variable
    ...
    close(fc)
    ##
    nContCov <- mcmcrun$nContinuousCovs
    nDiscrCov <- mcmcrun$nDiscreteCovs
    ...
    for(i in 1:length(nList)){
        ...
        names(phiList[[i]]) <- mcmcrun$discreteCovs
        dim(sigmaList[[i]]) <- c(nList[i], nContCov, nContCov)
        sigmaList[[i]] <- aperm(sigmaList[[i]])
        rownames(sigmaList[[i]]) <- mcmcrun$continuousCovs
        colnames(sigmaList[[i]]) <- mcmcrun$continuousCovs
        dim(muList[[i]]) <- c(nList[i], nContCov)
        muList[[i]] <- aperm(muList[[i]])
        rownames(muList[[i]]) <- mcmcrun$continuousCovs
    }
    ## Save the samples above
    mcmcoutput <- list(case=case, nList=nList, alphaList=alphaList, psiList=psiList, phiList=phiList, muList=muList, sigmaList=sigmaList, logPost=logPost, runtime=runtime)
    saveRDS(mcmcoutput, file=paste0('_mcmcoutput','.rds'))
    ##
    ## Plot diagnostic traces
    pdff('mcsummary')
    matplot(mcmcoutput$logPost[2,], type='l',ylim=range(mcmcoutput$logPost[2,],na.rm=T,finite=T),ylab='log-likelihood',main=paste0('chain: ',case))
    ##
    matplot(mcmcoutput$nList,type='l',ylim=range(mcmcoutput$nList,na.rm=T,finite=T),ylab='no. clusters', main=paste0('chain: ',case))
    ##
    if(length(mcmcoutput$alphaList) > 0){
    matplot(mcmcoutput$alphaList,type='l',ylim=range(mcmcoutput$alphaList,na.rm=T,finite=T),ylab='alpha', main=paste0('chain: ',case))}
    ##
    matplot(log(sapply(mcmcoutput$psiList,function(x)x[[1]])),type='l',ylab='first log-weight', main=paste0('chain: ',case))
    ##
    matplot(log(sapply(mcmcoutput$phiList,function(x)x[[1]][1])),type='l',ylab='first log-Bernoulli', main=paste0('chain: ',case))
    ##
    matplot(sapply(mcmcoutput$muList,function(x)x[1]),type='l',ylab='first mu', main=paste0('chain: ',case))
    ##
    matplot(log(sapply(mcmcoutput$sigmaList,function(x)x[1])),type='l',ylab='first log-sigma', main=paste0('chain: ',case))
    ##
    matplot(sapply(mcmcoutput$sigmaList,function(x)x[2]),type='l',ylab='second sigma', main=paste0('chain: ',case))
    ##
    for(i in c(1,3)){
        matplot(mcmcoutput$logPost[i,],type='l',ylim=range(mcmcoutput$logPost[i,],na.rm=T,finite=T), main=paste0('chain: ',case))
    }
    dev.off()
    ##
    mcmcoutput
...
pdf(file=paste0('testsampledensities_scaled_seq_',cases[j],'.pdf'),height=11.7,width=16.5*10)
## plot(NA,xlim=c(0,1),ylim=c(0,1),main='parameters')
## text(0,1,(paste0('transf = ','id','\ndim = ',dims,'\nmeanalpha = ',meanalpha,'\nsdalpha = ',sdalpha,'\nK = ',ka,'\nL = ',la,'\nM = ',mu,'\nresc = ',resc,'\nNclust = ',mean(clusters))),adj = c(0,1),cex=5)
##
for(asample in round(seq(1,1000,length.out=nplots))){
...
nTest <- 2000
## construct test set
testd <- alldata[unseldata, c('bin_RMSD',covNames), with=F]
testdata <- data.table()
for(val in rmsdVals){
...
priorP2 <- normalize(as.vector(table(alldata$bin_RMSD)))
unseldata <- setdiff(1:nrow(alldata), seldata)
nTest <- 2000
## construct test set
testd <- alldata[unseldata, c('bin_RMSD',covNames), with=F]
...
#### Evaluation
##
##discrMin <- sapply(alldata[,discreteCovs,with=F], min)-1
##
priorP <- rep(1,3)/3
unseldata <- setdiff(1:nrow(alldata), seldata)
##
...
plan(multisession, workers = 6L)
tstart <- Sys.time()
memtest0 <- t(sapply(rmsdVals,function(val){predictYtest(MCMCdata[[val]], alldata[30000+(1:ntdata)])}))
...
plan(multisession, workers = 6L)
tstart <- Sys.time()
memtest1 <- t(sapply(rmsdVals,function(val){predictYXtestsingle(MCMCdata[[val]], alldata[30000+(1:ntdata)])}))
...
plan(multisession, workers = 6L)
tstart <- Sys.time()
memtest2 <- aperm(predictYXtest(MCMCdata, alldata[30000+(1:ntdata)], rmsdVals),c(2,1,3))
Sys.time()-tstart
plan(sequential)

plan(multisession, workers = 6L)
tstart <- Sys.time()
memtest3 <- aperm(predictYXtests(MCMCdata, alldata[30000+(1:ntdata)], rmsdVals), c(2,1,3))
Sys.time()-tstart
plan(sequential)
...
plan(multisession, workers = 6L)
tstart <- Sys.time()
memtest <- predictYtest(MCMCdata[[1]], alldata[36000:36100])
Sys.time()-tstart
plan(sequential)
...
## 500, 6 threads: 1269.53  954.36 9520.95  
nTest <- 500
testdata <- alldata[unseldata, c('bin_RMSD',covNames), with=F]
testd <- data.table()
for(val in rmsdVals){
...
##
nTest <- 100
testdata <- alldata[unseldata,mcmcrun$covNames, with=F]
...
system.time(testres <- t(sapply(seq_len(nrow(testdata)), function(datum){
    ...
    c(datum['bin_RMSD']-discrMin['bin_RMSD'], colMeans(predictYpar2(mcmcrun,datum)))
})))
##
...
system.time(testres2 <- apply(alldata[unseldata[1:10], mcmcrun$covNames, with=FALSE], 1, function(x){
    c(x[1], colMeans(predictYpar(x[-1])))
}))

## user  system elapsed    7.37    0.05    7.43 
system.time(testres2 <- foreach(i=1:10, .combine=c)%do%{
    testyx <-  unlist(alldata[unseldata[i], mcmcrun$covNames, with=FALSE])
...
covNums <- c(1,3:8)
seldata <- sort(sample(nrow(alldata), ndata, replace=FALSE))
rm(datamc)
datamc <- as.data.frame(alldata[seldata, ..covNums])
...
##
#
system.time(mcmcrun <- profRegr(excludeY=TRUE, xModel='Mixed', nSweeps=10000, nBurn=2000, nFilter=10, data=datamc, nClusInit=10, covNames=c(discreteCovs,continuousCovs), discreteCovs=discreteCovs, continuousCovs=continuousCovs, nProgress=100, seed=222, output='_mc2output'))
##
...
##
pdff('cmsum_cont')
print(globalParsTrace(mcmcrun, parameters = "nClusters",plotBurnIn=F))
print(globalParsTrace(mcmcrun, parameters = "alpha",plotBurnIn=F))
...
continuousCovs <- names(datamc)[which(sapply(datamc, is.double)==TRUE)]
##
system.time(mcmcrun <- profRegr(excludeY=FALSE, yModel='Categorical', xModel='Normal', nSweeps=2000, nBurn=10, nFilter=2, data=datamc, nClusInit=2, outcome=yCov, covNames=c(discreteCovs,continuousCovs), discreteCovs=discreteCovs, continuousCovs=continuousCovs, nProgress=100, seed=222, output='_mc2output'))
##
pdff('cmsum_cont_y')
print(globalParsTrace(mcmcrun, parameters = "nClusters",plotBurnIn=F))
print(globalParsTrace(mcmcrun, parameters = "alpha",plotBurnIn=F))
...
## #
## system.time(mcmcrun <- profRegr(excludeY=FALSE, yModel='Categorical', xModel='Mixed', nSweeps=500, nBurn=100, nFilter=5, data=datamc, nClusInit=10, outcome='bin_RMSD', covNames=c(discreteCovs,continuousCovs), discreteCovs=discreteCovs, continuousCovs=continuousCovs, nProgress=100, seed=222, output='_mc2output'))
## #
## pdff('cmsum_y_discr')
## print(globalParsTrace(mcmcrun, parameters = "nClusters",plotBurnIn=F))
## print(globalParsTrace(mcmcrun, parameters = "alpha",plotBurnIn=F))
...
set.seed(149)
subsamplesize <- 2^9
samplescale <- max(foreach(i=(-1):1, .combine=c)%do%{length(which(alldata$bin_RMSD==i))})/subsamplesize

plotpoints <- foreach(i=(-1):1)%do%{
    matr <- as.matrix(alldata[which(alldata$bin_RMSD==i),..feats])
...
rmsdData <- alldata[[totFeatures+1]]
...
## rmsdData <- unlist(alldata[[rmsdCol]])
## for(i in okFeatures[-rmsdCol]){
##     datum <- unlist(alldata[[i]])
...
data2 <- cbind(data.table(logrmsd=log(alldata$rmsd)),
               alldata[,5:ncol(alldata)])
...
plan(multisession, workers = 4L)
foreach(i=1:4)%dopar%{
    outputdir <- paste0('test',i)
    if(!dir.exists(outputdir)){dir.create(outputdir)}
    setwd(outputdir)
    a <- 1
    save(list=c('a'), file=paste0('test.R'))
    i
}
plan(sequential)
...
## Author: PGL  Porta Mana
## Created: 2021-03-20T10:07:17+0100
## Last-Updated: 2021-09-12T15:54:33+0200
################
## Script for direct regression, continuous RMSD
################
