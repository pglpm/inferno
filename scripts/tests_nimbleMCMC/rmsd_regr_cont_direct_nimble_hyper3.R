...
inits <- list(
    ...
    sizeD=matrix(rbinom(n=ndcovs*nclusters, prob=meansdcovs/maxdcovs, size=maxdcovs), nrow=ndcovs, ncol=nclusters),
...
bayesnet <- nimbleCode({
    ...
    for(adatum in 1:nData){
        ...
        for(acov in 1:nDcovs){
            Y[adatum,acov] ~ dnbinom(prob=probD[acov,C[adatum]], size=sizeD[acov,C[adatum]])
        }
    }
})
##
