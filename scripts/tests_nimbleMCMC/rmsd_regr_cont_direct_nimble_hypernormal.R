...
## Last-Updated: 2021-09-18T11:52:13+0200
################
...
discreteCovs <- c() ##covNames[sapply(covNames, function(x){is.integer(alldata[[x]])})]
...
covCmeans <- apply(alldata[1:ndata,..continuousCovs],2,mean)
covCvars <- apply(alldata[1:ndata,..continuousCovs],2,var)
##
##
##
constants <- list(
...
inits <- list(
    ...
    meanCmean=rnorm(n=ncvars, mean=covCmeans, sd=sqrt(covCvars)),
    meanCtau=rgamma(n=ncvars, shape=0.5, rate=covCvars/2),
    tauCshape=rinvgamma(n=ncvars, shape=0.5, scale=0.5),
    tauCrate=rgamma(n=ncvars, shape=0.5, rate=1/(covCvars*2)),
    ##
    q=rdirch(n=1, alpha=rep(1, nclusters)),
    meanC=matrix(rnorm(n=ncvars*nclusters, mean=covCmeans, sd=sqrt(covCvars)), nrow=ncvars, ncol=nclusters),
    tauC=matrix(rgamma(n=ncvars*nclusters, shape=0.5, rate=covCvars/2), nrow=ncvars, ncol=nclusters),
    ##
...
bayesnet <- nimbleCode({
    ...
    alphad[1:nClusters] <- qalpha/nClusters
    q[1:nClusters] ~ ddirch(alpha=alphad[1:nClusters])
    for(acluster in 1:nClusters){
        for(avar in 1:nCvars){
            meanC[avar,acluster] ~ dnorm(mean=meanCmean[avar], tau=meanCtau[avar])
            tauC[avar,acluster] ~ dgamma(shape=tauCshape[avar], rate=tauCrate[avar])
        }
    }
    for(avar in 1:nCvars){
        meanCmean[avar] ~ dnorm(mean=mean0[avar], sd=sqrt(var0[avar]))
        meanCtau[avar] ~ dgamma(shape=0.5, rate=var0[avar]/2)
        tauCshape[avar] ~ dinvgamma(shape=0.5, scale=0.5)
        tauCrate[avar] ~ dgamma(shape=0.5, rate=1/(2*var0[avar]))
    }
    ##
    for(adatum in 1:nData){
        C[adatum] ~ dcat(prob=q[1:nClusters])
        for(avar in 1:nCvars){
            X[adatum,avar] ~ dnorm(mean=meanC[avar,C[adatum]], tau=tauC[avar,C[adatum]])
        }
    }
})
##
source('functions_rmsdregr_nimble_normals.R')
...
confmodel <- configureMCMC(Cmodel, monitors=c('q','meanC', 'tauC'))
...
mcsamples <- runMCMC(Cmcmcsampler, nburnin=0, niter=1000, thin=1, inits=initsFunction, setSeed=149)
...
print(totaltime)
## 7 vars, 6000 data, 100 cl, 100 iter: 4.860164 mins
...
loglikelihood <- llSamples(X=dat, parmList=parmList)
plan(sequential)
