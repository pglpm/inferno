...
## Last-Updated: 2021-09-18T16:28:28+0200
################
...
probsdcovs <- 1 - varsdcovs/meansdcovs
sizesdcovs <- meansdcovs^2/(meansdcovs - varsdcovs)
##
##
constants <- list(
    nClusters=nclusters,
    ...
    varsCcovs=varsccovs,
    probsDcovs=probsdcovs,
    sizesDcovs=sizesdcovs
...
inits <- list(
    ...
    meanCmean=rnorm(n=nccovs, mean=0, sd=1),
    meanCtau=rgamma(n=nccovs, shape=0.5, rate=0.5),
    tauCshape=rinvgamma(n=nccovs, shape=0.5, scale=0.5),
    tauCrate=rgamma(n=nccovs, shape=0.5, rate=0.5),
    probDshape1=rinvgamma(n=ndcovs, shape=0.5, scale=0.5),
    probDshape2=rinvgamma(n=ndcovs, shape=0.5, scale=0.5),
    sizeDshape=rinvgamma(n=ndcovs, shape=0.5, scale=0.5),
    sizeDscale=rgamma(n=ndcovs, shape=0.5, rate=1),
    ##
    q=rdirch(n=1, alpha=rep(1, nclusters)),
    meanC=matrix(rnorm(n=nccovs*nclusters, mean=0, sd=1), nrow=nccovs, ncol=nclusters),
    tauC=matrix(rgamma(n=nccovs*nclusters, shape=0.5, rate=0.5), nrow=nccovs, ncol=nclusters),
    probD=matrix(rbeta(n=ndcovs*nclusters, shape1=1, shape2=1), nrow=ndcovs, ncol=nclusters),
    sizeD=matrix(rinvgamma(n=ndcovs*nclusters, shape=0.5, scale=0.5), nrow=ndcovs, ncol=nclusters),
    ##
    ...
    for(acluster in 1:nClusters){
        ...
        for(acov in 1:nDcovs){
            ...
            sizeD[acov,acluster] ~ dbinom(prob=sizeDprob[acov], size=sizeDsize[acov])
                shape=sizeDshape[acov], scale=sizeDscale[acov])
        }
    }
    for(acov in 1:nCcovs){
        ...
        tauCrate[acov] ~ dgamma(shape=0.5, rate=1/(2*varsCcovs[acov]))
    }
    for(acov in 1:nDcovs){
