...
## Last-Updated: 2021-11-24T05:58:59+0100
################
...
x=c(64, 60, 59, 65, 64, 62, 54, 68, 67, 79, 45, 48, 59, 65, 87, 49, 46, 46, 97, 36, 67, 56, 62, 65, 65, 81, 83, 71),
...
seed <- 149
baseversion <- 'mmivexample_0M12S1_'
...
for(acov in covNames){
        dataQuantiles[[acov]] <- quantile8(alldata[[acov]], prob=c(0.005,0.995))
...
list(
    ...
    sizeDprob1=rep(0.999999, ndcovs),
    sizeDsize1=rep(0.999999/(1-0.999999),ndcovs),
    ##
    ##
...
legend('topright', legend=c(
       ...
       bty='n', cex=1.25)
##
tplot(mdh1$mid, mdh1$density, col=1, xlab='mean', ylab='probability density', main='predicted means of groups A and B', xlim=range(c(mdh1$mid,mdh2$mid)), ylim=c(0,max(c(mdh1$density,mdh2$density))))
tplot(mdh2$mid, mdh2$density, col=2, xlab='mean group B', ylab='probability density', main='predicted mean of group A', xlim=range(c(mdh1$mid,mdh2$mid)), ylim=c(0,max(c(mdh1$density,mdh2$density))), add=T)
##
tplot(mdh$mid, mdh$density, col=3, xlab='(mean group A) - (mean group B)', ylab='probability density', main='predicted difference between means of groups A and B')
...
##
tplot(vdh1$mid, vdh1$density, col=1, xlab='variance', ylab='probability density', main='predicted variances of groups A and B', #xlim=quantile8(vdists, c(2.5,97.5)/100),
      ylim=c(0,max(c(vdh1$density,vdh2$density))))
tplot(vdh2$mid, vdh2$density, col=2, xlab='mean group B', ylab='probability density', main='predicted mean of group A', xlim=range(c(vdh1$mid,vdh2$mid)), ylim=c(0,max(c(vdh1$density,vdh2$density))), add=T)
##
tplot(vdh$mid, vdh$density, col=4, xlab='(variance group A) - (variance group B)', ylab='probability density', main='predicted difference between variances of groups A and B') #xlim=quantile8(vardiffs,c(0.5,99.5)/100))
...
legend('topleft', legend=paste0(signif(quantile8(medians1-medians2,c(1)/8)*2,2),' < difference < ',signif(quantile8(medians1-medians2,c(7)/8)*2,2),'\nwith 75% probability','\n\n',
                signif(quantile8(medians1-medians2,c(5)/100)*2,2),' < difference < ',signif(quantile8(medians1-medians2,c(95)/100)*2,2),'\nwith 90% probability'                ), bty='n', cex=1.5)
dev.off()
