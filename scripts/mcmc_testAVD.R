...
## Last-Updated: 2022-12-13T15:37:49+0100
#########################################
...
baseversion <- '_testAVD' # *** ## Base name of output directory
## datafile <- 'testdataS1.csv'#'ingrid_data_nogds6.csv' #***
datafile <- 'ingrid_data_nogds6.csv' #***
predictorfile <- 'predictorsAVD.csv'
predictandfile <- 'predictorsAVD.csv'
varinfofile <- 'varinfoAVD.rds'
...
while(continue){
    ...
    if(stage==0){# burn-in stage
        ...
        newmcsamples <- Cmcsampler$run(niter=niter+1, thin=thin, thin2=1, nburnin=1024, time=T)
...
##    saveRDS(finalstate2list(finalstate, realVars=realVars, integerVars=integerVars, categoryVars=categoryVars, binaryVars=binaryVars, compoundgamma=compoundgamma), file=paste0(dirname,'_finalstate-R',basename,'--',mcmcseed,'-',stage,'.rds'))
    ...
    maxv <- varinfo[['max']][v]
    nnv <- varinfo[['n']][v]
    Xgrid <- seq(minv,maxv,length.out=nnv)
    Xrights <- qnorm((1:nnv)/nnv)
    Xlefts <- qnorm(((1:nnv)-1)/nnv)
    subsamples <- 1:nrow(newmcsamples)
    testprob <- sapply(subsamples, function(sam){
        rowSums(sapply(1:nclusters, function(clu){
            (
            pnorm(q=Xrights[Xgrid+1],
                  mean=newmcsamples[sam,paste0('Imean[1, ',clu,']')],
                  sd=sqrt(newmcsamples[sam,paste0('Ivar[1, ',clu,']')])
                  ) -
            pnorm(q=Xlefts[Xgrid+1],
                  mean=newmcsamples[sam,paste0('Imean[1, ',clu,']')],
                  sd=sqrt(newmcsamples[sam,paste0('Ivar[1, ',clu,']')])
                  )
            ) *
...
##
    testprob2 <- samplesFDistribution(Y=matrix(Xgrid,ncol=1,dimnames=list(NULL,v)), X=NULL, varinfo=varinfo, mcsamples=newmcsamples, jacobian=T)
    tplot(y=list(rowMeans(testprob), rowMeans(testprob2)))
    hist <- thist(data.matrix(data0)+1,n='i')
    tplot(y=hist$density,add=T)

        finalstate <- as.matrix(Cmcsampler$mvSamples2)

    extre <- which(data.matrix(data0)+1==16)
    tplot(y=finalstate[,paste0('Icont[',extre[5],', 1]')])
