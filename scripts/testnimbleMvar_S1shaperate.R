...
initsFunction <- function(){
    ...
    Rrate <- matrix(rinvgamma(n=nvar*nclusters, shape=shape0[Shapeindexlo], rate=Rvar1), nrow=nvar, ncol=nclusters)
...
finitemix <- nimbleCode({
    ...
    for(k in 1:nclusters){
        for(v in 1:nvar){
            ...
            Rrate[v, k] ~ dinvgamma(shape=shape0[Shapeindexlo[v]], rate=Rvar1[v])
...
##
pdff(paste0('testmchyper-mean_var-sd_1shaperate'))
for(i in 1000:990){
