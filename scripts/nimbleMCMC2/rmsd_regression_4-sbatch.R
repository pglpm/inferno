...
## Last-Updated: 2021-10-05T19:51:28+0200
################
...
version <- paste0('postHM', stage)
gc()
totalruntime <- Sys.time()
if(stage==0){
    mcsamples <- runMCMC(Cmcmcsampler, nburnin=1, niter=niter+1, thin=1, inits=initsFunction, setSeed=149)
}else{
    Cmcmcsampler$run(niter=niter, thin=1, reset=FALSE, resetMV=TRUE)
