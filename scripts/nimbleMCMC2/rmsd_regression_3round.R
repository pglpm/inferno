...
inits <- list(
    ...
    sizeDpar1=1 + 0*maxdcovs,
    sizeDpar2=1/maxdcovs,
    ##
    ...
    csizeD=matrix(maxdcovs, nrow=ndcovs, ncol=nclusters),
...
bayesnet <- nimbleCode({
    ...
    for(acluster in 1:nClusters){
        ...
        for(acov in 1:nDcovs){
            ...
            csizeD[acov,acluster] ~ dgamma(shape=sizeDpar1[acov], rate=sizeDpar2[acov])
            sizeD[acov,acluster] <- ceiling(csizeD[acov,acluster])
        }
    }
    if(posterior){
