...
initsFunction <- function(){
    ...
    Shapeindexlo <- runif(nvar, minshape, maxshape)
    Shapeindexhi <- runif(nvar, minshape, maxshape)
    ##
...
todelete <- Cmcsampler$run(niter=15000*2, thin=150, thin2=1, nburnin=0, time=TRUE, reset=FALSE, resetMV=FALSE)
...
q <- extraDistr::rdirichlet(n=prc,alpha=matrix(2^alphas/nclusters,nrow=prc,ncol=nclusters))
##
...
sds <- array(sqrt(nimble::rinvgamma(prc*2*nclusters, shape=2^sdssl, rate=
                                                           nimble::rinvgamma(prc*2*nclusters, shape=2^sdssh, rate=sdsr))), dim=c(prc,2,nclusters))
