...
initsFunction <- function(){
    ...
    Shapelo <- rinvgamma(nvar, shape=minshape, scale=maxshape)
    Shapehi <- rinvgamma(nvar, shape=minshape, scale=maxshape)
    ##
...
thistime <- Sys.time()
mcsamples <- foreach(cor=1:ncores, .combine=rbind, .packages='nimble')%dorng%{
    thisseed <- 890+cor
##  
...
todelete <- Cmcsampler$run(niter=1000*10+0, thin=1000, thin2=1, nburnin=0, time=TRUE, reset=FALSE, resetMV=FALSE)
...
sdssl <- nimble::rinvgamma(prc*2, shape=minshape, scale=maxshape)
sdssh <- nimble::rinvgamma(prc*2, shape=minshape, scale=maxshape)
...
##
pdff(paste0('_testxigsprior2D_Mvar_S2shapes_A',minalpha,'_',maxalpha,'_S',minshape,'_',maxshape,'_N',npoints),apaper=3)
##
...
##
par(mfrow=c(1,1))
for(i in round(seq(nrow(mcsamples),1,length.out=nrow(mcsamples)))){
