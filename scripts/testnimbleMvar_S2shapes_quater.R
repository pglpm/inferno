...
initsFunction <- function(){
    ...
    Rrate <- matrix(rinvgamma(n=nvar*nclusters, shape=Shapeindexhi, rate=Rvar1), nrow=nvar, ncol=nclusters)
    Rvar <- matrix(rinvgamma(n=nvar*nclusters, shape=Shapeindexlo, rate=Rrate), nrow=nvar, ncol=nclusters)
...
finitemix <- nimbleCode({
    ...
    for(k in 1:nclusters){
        for(v in 1:nvar){
            ...
            Rrate[v, k] ~ dinvgamma(shape=Shapeindexhi[v], rate=Rvar1[v])
            Rvar[v, k] ~ dinvgamma(shape=Shapeindexlo[v], rate=Rrate[v, k])
        }
    }
    for(d in 1:npoints){
...
##
pdff(paste0('xuprior2D_Mvar_S2shapes_A',minalpha,'_',maxalpha,'_S',minshape,'_',maxshape,'_N',npoints),apaper=3)
