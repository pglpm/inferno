...
initsFunction <- function(){
    probalpha0 <- numeric(nalpha) #+ 2^-20
    probalpha0[(minalpha:maxalpha)+nalpha2+1] <- rep(1/(maxalpha-minalpha+1),maxalpha-minalpha+1)
    walpha0 <- matrix(2^((-nalpha2):nalpha2)/nclusters, nrow=nalpha, ncol=nclusters)
    ...
    Shapeindexlo <- extraDistr::rcat(n=nvar, prob=probshape0)
    Shapeindexhi <- extraDistr::rcat(n=nvar, prob=probshape0)
    ##
    Rmean <- matrix(rnorm(n=nvar*nclusters, mean=Rmean1, sd=sqrt(Rvarm1)), nrow=nvar, ncol=nclusters)
    Rrate <- matrix(rinvgamma(n=nvar*nclusters, shape=wshape0[Shapeindexhi], rate=Rvar1), nrow=nvar, ncol=nclusters)
    Rvar <- matrix(rinvgamma(n=nvar*nclusters, shape=wshape0[Shapeindexlo], rate=Rrate), nrow=nvar, ncol=nclusters)
...
finitemix <- nimbleCode({
    ...
    for(v in 1:nvar){
        ...
        ##
        Shapeindexlo[v] ~ dcat(prob=probshape0[1:nshape])
        Shapeindexhi[v] ~ dcat(prob=probshape0[1:nshape])
    ...
    for(k in 1:nclusters){
        for(v in 1:nvar){
            ...
            Rrate[v, k] ~ dinvgamma(shape=wshape0[Shapeindexhi[v]], rate=Rvar1[v])
            Rvar[v, k] ~ dinvgamma(shape=wshape0[Shapeindexlo[v]], rate=Rrate[v, k])
        }
    }
    for(d in 1:npoints){
...
confnimble <- configureMCMC(Cfinitemixnimble,
                            monitors=c('Alphaindex', 'K',  'W', 'Rmean', 'Rvar',
                                       'Rvarm1',
                                       ##'Rrate',
                                       'Shapeindexhi',
                                       'Shapeindexlo',
...
ord0 <- confnimble$getSamplerExecutionOrder()
## confnimble$setSamplerExecutionOrder(rev(c(ord0[length(ord0)], ord0[-length(ord0)])))
confnimble$setSamplerExecutionOrder(rev(ord0))

##confnimble$setSamplerExecutionOrder(c(ord0[length(ord0)], ord0[-length(ord0)]))
...
todelete <- Cmcsampler$run(niter=100, thin=1, thin2=1, nburnin=0, time=TRUE, reset=TRUE, resetMV=TRUE)
mcsamples <- as.matrix(Cmcsampler$mvSamples)


pdff('with_categorical')
    par(mfrow=c(2,1), mar = c(0,0,0,0))
tplot(y=apply(mcsamples,1,function(xxx){min(log(xxx[extract('W')]))}),
      xlab=NA, xlabels=NA, ylab='min(log(W))',
      mar=c(1,6,4,NA), main='with categorical')
tplot(y=mcsamples[,'Alphaindex'], col=2,
      xlab='iteration', ylab='Alphaindex', mar=c(NA,6,1,NA))
plotquantiles(x=1:100,y=cbind(rep(1,100),rep(3.9,100)))
dev.off()


min(mcsamples[,extract('W')])
...
##
## >            K         Rvar        Rmean        Rrate            W Shapeindexlo 
##    57.675993    42.821855    29.588006     1.970837     1.410044     1.388503 
## Shapeindexhi   Alphaindex       Rvarm1      Rratem1 
##     1.341689     0.533981     0.196411     0.026036 


uk <- apply(mcsamples,1,function(rr){length(unique(rr[extract('K')]))})
table(uk)
print('alpha')
mean(log2(walpha0[mcsamples[,extract('Alphaindex')]]))
table(log2(walpha0[mcsamples[,extract('Alphaindex')]]))
print('shapehi')
mean(log2(wshape0[mcsamples[,extract('Shapeindexhi')]]))
table(wshape0[mcsamples[,'Shapeindexhi[1]']])
table(wshape0[mcsamples[,'Shapeindexhi[2]']])
print('shapelo')
mean(log2(wshape0[mcsamples[,extract('Shapeindexlo')]]))
table(wshape0[mcsamples[,'Shapeindexlo[1]']])
table(wshape0[mcsamples[,'Shapeindexlo[2]']])
...
sdssl <- sample(rep(shape0,2), size=prc*2, replace=T)
sdssh <- sample(rep(shape0,2), size=prc*2, replace=T)
sdsr <- 1#nimble::rinvgamma(prc*2, shape=baseshape1, rate= nimble::rinvgamma(prc*2, shape=baseshape1, rate=1))
##
pdff(paste0('prior2D_Mvar_S2shapes_A',minalpha,'_',maxalpha,'_S',minshape,'_',maxshape),apaper=3)
