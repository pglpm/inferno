...
## Last-Updated: 2021-07-13T23:53:41+0200
################
...
tanimoto2y <- function(x){
    qt(x, df=df0)*sdtani+mutani # correct compared with study_prior_bayes_regr.nb
}
##
## correct compared with study_prior_bayes_regr.nb
Dtanimoto2y <- function(x){
    sqrt(1 + (tanimoto2y(x)^2)/(df0*sdtani^2))^(df0+1) * sqrt(df0) * sdtani * beta(df0/2, 0.5)
}
##
