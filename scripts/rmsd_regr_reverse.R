...
## standardize the continuous 'sasa' features
...
for(elem in colnames(data)[indx]){
    datum <- qlogis(data[[elem]])
    eps <- max(diff(sort(unique(datum[abs(datum)!=Inf]))))
    datum[datum==Inf] <- max(datum[abs(datum)!=Inf])+3*eps
    datum[datum==-Inf] <- min(datum[abs(datum)!=Inf])-3*eps
    data[, elem] <- datum
}
names(data)[indx] <- paste0('logit_',names(data)[indx])
##
...
for(j in 1:length(sampledata)){
    sd <- sampledata[[j]]
    matplot(sd$logPost[2,], type='l',ylim=range(sd$logPost[2,],na.rm=T,finite=T),ylab='log-likelihood',col=mypalette[j])
    }
for(j in 1:length(sampledata)){
    sd <- sampledata[[j]]
matplot(sd$nList,type='l',ylim=range(nList,na.rm=T,finite=T),ylab='no. clusters',col=mypalette[j])
}
for(j in 1:length(sampledata)){
    sd <- sampledata[[j]]
    matplot(sd$alphaList,type='l',ylim=range(alphaList,na.rm=T,finite=T),ylab='alpha',col=mypalette[j])
    }
for(j in 1:length(sampledata)){
    for(i in c(1,3)){matplot(sd$logPost[i,],type='l',ylim=range(logPost[i,],na.rm=T,finite=T),col=mypalette[j])}
    }
dev.off()
#plan(sequential)
...
normalize(foreach(val=rmsdVals+1, .combine=c)%:%foreach(sample=seq_along(dataobj[[val]]$nList), .combine='+', .inorder=FALSE)%dopar%{
        sum(exp(log(dataobj[[val]]$psiList[[sample]]) + sapply(seq_len(dataobj[[val]]$nList[sample]),function(j){
            sum(log(sapply(dC, function(elem){
                dataobj[[val]]$phiList[[sample]][[elem]][x[elem]-discrMin[elem], j]
            }))) +
...
##
#
system.time(testmc <- profRegr(excludeY=TRUE, xModel='Mixed', nSweeps=10000, nBurn=2000, nFilter=10, data=datamc, nClusInit=10, covNames=c(discreteCovs,continuousCovs), discreteCovs=discreteCovs, continuousCovs=continuousCovs, nProgress=100, seed=222, output='-mc2output'))
##
...
continuousCovs <- names(datamc)[which(sapply(datamc, is.double)==TRUE)]
##
system.time(testmc <- profRegr(excludeY=FALSE, yModel='Categorical', xModel='Normal', nSweeps=2000, nBurn=10, nFilter=2, data=datamc, nClusInit=2, outcome=yCov, covNames=c(discreteCovs,continuousCovs), discreteCovs=discreteCovs, continuousCovs=continuousCovs, nProgress=100, seed=222, output='-mc2output'))
##
pdff('cmsum_cont_y')
print(globalParsTrace(testmc, parameters = "nClusters",plotBurnIn=F))
print(globalParsTrace(testmc, parameters = "alpha",plotBurnIn=F))
...
## #
## system.time(testmc <- profRegr(excludeY=FALSE, yModel='Categorical', xModel='Mixed', nSweeps=500, nBurn=100, nFilter=5, data=datamc, nClusInit=10, outcome='bin_RMSD', covNames=c(discreteCovs,continuousCovs), discreteCovs=discreteCovs, continuousCovs=continuousCovs, nProgress=100, seed=222, output='-mc2output'))
## #
## pdff('cmsum_y_discr')
## print(globalParsTrace(testmc, parameters = "nClusters",plotBurnIn=F))
## print(globalParsTrace(testmc, parameters = "alpha",plotBurnIn=F))
...
pdff('jointplotslog_RMSD_fabiodata')
