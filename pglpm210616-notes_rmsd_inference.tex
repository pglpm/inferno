...
%% Last-Updated: 2021-07-04T16:40:52+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
Our problem is to infer $y_{0}$ given: $x_{0}$ for a new datapoint, known
pairs
$(\wy,\wx) \defd \bigl((y_{1}, x_{1}), (y_{2}, x_{1}), \dotsc, (y_{N},
x_{N})\bigr)$ for other $N$ datapoints, and additional facts and hypotheses
$H$. Additional hypotheses are unavoidable since we are making an
extrapolation. Our uncertainty about $y_{0}$ is expressed by a probability
(density)
\begin{equation}
  \label{eq:main_prob}
  \p(y_{0} \| x_{0},\ \wy,\wx,\ H) \ ,
...
The first question we must ask in approaching this problem is how much our
uncertainty about $y_{0}$ depends on the relative frequencies for previous
datapoints $(\wy,\wx)$. For example, if $y_{0}$ is hand-picked by someone,
then knowledge of the frequencies for $(\wy,\wx)$ is irrelevant. But even
in such a case it might be that the reversed probability
...
given $x_{0}$, or vice versa. Or using Fisherian \citey[\sects~II.4,
...
For the direct case~\eqref{eq:main_prob} we shall consider the \rmsd\ $y$
as a continuous variable, mapped to a log-scale.

The assumption in this case is that $y_{0}$ is exchangeable given $x_{0}$,
but $x_{0}$ is not exchangeable given the known datapoints $\wx$. This a
sensible assumption -- a fact indeed -- if we are actually \emph{choosing}
the specific value of $x_{0}$, as we do in testing for example.

The calculation of $\p(y_{0} \| x_{0},\ \wy,\wx,\ H)$ is actually done via
the calculation of the joint probability $\p(y_{0}, x_{0} \| \wy,\wx,\ H)$
through
\begin{equation}
  \label{eq:cond_prob}
  \p(y_{0} \| x_{0},\ \wy,\wx,\ H) =
