...
%% Last-Updated: 2021-08-05T19:52:31+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
Our problem is to infer the value $r_{0}$ of a new datapoint, given:
\begin{itemize}
\item features $x_{0}$ for the new datapoint,
\item known pairs
...
possible values: \enquote*{good} ($0\ \textrm{\AA}\le r \le 2\
\textrm{\AA}$), \enquote*{inconclusive} ($2\ \textrm{\AA} < r < 3\
\textrm{\AA}$),  \enquote*{bad} ($3\ \textrm{\AA} \le r$). 


Our uncertainty about $r_{0}$ is expressed by the probability
\begin{equation}
  \label{eq:main_prob}
  \p(r_{0} \| x_{0},\ \br,\bx,\ H) \ ,
\end{equation}
...
The first question we must ask for such quantification is this: How much
are the various relative frequencies for the known datapoints $(\br,\bx)$
relevant to our uncertainty about $r_{0}$? To understand the meaning of
this question, consider the case where $r_{0}$ is
\emph{hand-picked} to be \enquote*{good}: then the frequency of
$r_{0}=\text{\enquote*{good}}$ in the known dataset is obviously irrelevant
to its \enquote{prediction}.
...
probability $\p(x_{0} \| r_{0},\ \br,\bx,\ H)$ \emph{does} depend on the
frequencies of previous datapoints, and it can be used to
calculate~\eqref{eq:main_prob} as
\begin{equation}
  \label{eq:rev_prob}
  \p(r_{0} \| x_{0},\ \br,\bx,\ H) =
  \frac{\p(x_{0} \| r_{0},\ \br,\bx,\ H)\ \p(r_{0} \| H)}{
    \sum_{r_{0}}\p(x_{0} \| r_{0},\ \br,\bx,\ H)\ \p(r_{0} \| H)}
\ .
\end{equation}
...
$\p(r_{0} \| H)$ using extra-data facts or hypotheses.

In technical terms we are asking whether $r_{0}$ is \emph{exchangeable}
given $x_{0}$, or vice versa. Using Fisherian \citey[\sects~II.4,
IV.1]{fisher1956} parlance we are asking whether $r_{0}$ should be
...
$x_{0} \rightsquigarrow r_{0}$ or $r_{0} \rightsquigarrow x_{0}$
...
three rules of the probability calculus. For example, if $r_{0}$ is assumed
...
\begin{equation}
  ...
  \p(r_{0} \| x_{0},\ \br,\bx,\ H) =
  \frac{
    \int F_{r_{0}\|x_{0}} \ F_{r_{1},x_{1}} \  \dotsm \ 
...
$(\br,\bx)$, the probability distribution for $r_{0}$ given $x_{0}$
...
\parencites{mackay1992,mackay1992b}, for example a value of $r_{0}$ that
...
In the direct case the assumption is that $r_{0}$ is exchangeable given
$x_{0}$, but $x_{0}$ is not exchangeable given the known datapoints $\bx$.
...
The probability $\p(r_{0} \| x_{0},\ \br,\bx,\ H)$ is then given by
\eqn~\eqref{eq:main_prob_exch}, which can be rewritten this way:
\begin{equation}
  \label{eq:direct_case_main}
  \begin{gathered}
  \p(r_{0} \| x_{0},\ \br,\bx,\ H) =
  \int F_{r_{0}\|x_{0}} \ \p(\yF \| \br, \bx,\ H)\ \di\yF
  \\[\jot]
  \text{with}\quad
  \p(\yF \| \br, \bx,\ H) =
  \frac{
...
approximated by a sum of values of $F_{r_{0} \| x_{0}}$ sampled from the
distribution $\p(\yF \| \br, \bx,\ H)$ via Markov-chain Monte Carlo
...
$r_{0}$, but $r_{0}$ is not exchangeable given the known datapoints $\br$.
This is again a sensible assumption if we are choosing the specific value
of $x_{0}$, as we do in testing situations, because then $r_{0}$ cannot be
considered to come from some unsystematic process, if it is has some causal
connections with $x_{0}$. The values $(\br,\bx)$ are considered
exchangeable.

The probability $\p(x_{0} \| r_{0},\ \br,\bx,\ H)$ is obtained analogously
...
\begin{equation}
  ...
  \begin{gathered}
  \p(x_{0} \| r,\ \br,\bx,\ H) =
\int F_{x_{0}\|r} \ \p(\yF_{\bm{x\mid}r} \| \bx,\ H)\ \di\yF_{\bm{x\mid}r}
  \\[\jot]
  \p(\yF_{\bm{x\mid}r} \| \bx,\ H) =
  \frac{
    \ F_{x_{1'}\| r} \  \dotsm \ 
...
\end{gathered}
\end{equation}
where the conditionals contain only $\bx$ values associated with that
...
% %\appendixpage
\appendix
\mynote{\pencil\ These are just working notes}
