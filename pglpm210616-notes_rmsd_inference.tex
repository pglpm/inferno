...
%% Last-Updated: 2021-07-05T23:19:32+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
we'll study both possibilities mentioned above. At the end we'll compare
the results and discuss which assumption makes more sense in various
circumstances, for example during testing.
...
\emph{choosing} the specific value of $x_{0}$, as we do in testing
situations for example. The values $(\wy,\wx)$ are considered exchangeable.
...
parametrize it and at the same time choose a density $\p(\yF \| H)$ is by
...
$F_{y,x} = \sum_{i}w_{i} K(y,x \| \theta_{i})$. The density $\p(\yF \| H)$
is therefore defined over the possible infinite tuples
$(w_{i},\theta_{i})$. A Dirichlet process is chosen for such density. An
example of this approach is analysed and used by
\textcites{muelleretal1996}.

In the present case the kernel is the product of a multivariate normal
distribution for $y$ and for the continuous features in $x$, and a
Dirichlet distribution for the discrete features in $x$
\parencites[\sect~3.3]{liveranietal2015}.
