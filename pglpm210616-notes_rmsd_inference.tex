...
%% Last-Updated: 2021-08-08T15:42:53+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
%\prod_{i=1}^{N} F_{r_{i}\,x_{i}} \ \di\yF \ .
  ...
  % }
\end{equation}
to be combined with \eqn~\eqref{eq:post_prob_F}. This formula is valid when
we assume exchangeability in $r$ given $x$, but not in $x$.
  
\item\label{item:I_iii} $H_{\textrm{I}}$ and $H_{\textrm{iii}}$. % Combining
  % \eqns~\eqref{eq:p_F_unfactorizable}, \eqref{eq:post_prob_F},
  ...
  % }
  \end{equation}
  to be combined with \eqn~\eqref{eq:post_prob_F}. This formula is valid
  when we assume exchangeability in $r$ given $x$, and also in $x$.

\item\label{item:I_i} $H_{\textrm{I}}$ and $H_{\textrm{i}}$. Considering
  \eqn~\eqref{eq:p_xr_bayes} we find
  \begin{equation}
...
\item\label{item:II_i} $H_{\textrm{II}}$ and $H_{\textrm{i}}$. Considering
  \eqns~\eqref{eq:p_xr_bayes} and~\eqref{eq:post_prob_Fxcondr} we find
  \begin{equation}
...
%\prod_{i=1}^{N} F_{r_{i}\,x_{i}} \ \di\yF \ .
  ...
  % }
\end{equation}
to be combined with \eqn~\eqref{eq:post_prob_Fxcondr}. This formula is
valid when we assume exchangeability in $x$ given $r$, but not in $r$, and
vanishing mutual information between marginal and conditional long-run
frequencies $\yF_{r\q}$ and $\yFr$.
\end{enumerate}
...
by a finite number $T$ of samples $\set{\yF^{(t)}}$ and
$\set{\yFr^{(t)}}$ drawn from them via Markov-chain Monte Carlo
methods, and integration with respect to them is approximated by summation
over such samples. The formulae obtained in the previous section take the
...
\item\label{item:approx_I_ii} $H_{\textrm{I}}$ and $H_{\textrm{ii}}$:
  \begin{equation}
    ...
    \p(\ro \| \xo,\ \rd,\xd, H_{\textrm{ii\,I}}) 
    \approx  \sum_{t} F^{(t)}_{\ro\mid\xo} \ ,\qquad
...
\item\label{item:approx_II_i} $H_{\textrm{II}}$ and $H_{\textrm{i}}$:
  \begin{multline}
    ...
    \yF^{(t)}\text{ drawn from }\p(\yFr\|\rd,\xd, H_{\textrm{II}})\,\di\yFr \ .
\end{multline}
where the conditional frequencies $\set{\yFr^{(t)}}$ are drawn
directly, rather than being calculated from samples of joint frequencies
$\set{\yF^{(t)}}$ as in the previous three formulae.
\end{enumerate}
