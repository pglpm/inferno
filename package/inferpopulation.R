...
inferpopulation <- function(data, metadata, outputdir, nsamples = 0,
  ...
  data_no_na <- which(apply(data, 1,
    function(xx) {
      !any(is.na(xx))
    }
  )
  )
  nrows_no_na <- length(data_no_na)
  if (is.numeric(loglikelihood) && loglikelihood > 1 && nrows_no_na > 1) {
    # Make sure loglikelihood is not larger than the
    # nr of rows without missing data
    loglikelihood <- min(round(loglikelihood), nrows_no_na)
  } else if (is.logical(loglikelihood) && loglikelihood && nrows_no_na > 1) {
    # If loglikelihood is set to TRUE,
    # set length equal to nr of rows without missing data
    loglikelihood <- nrows_no_na
  } else {
    loglikelihood <- FALSE
  }
  ...
  if(timestampdir){
      timestamp <- paste0(
                          '-V', nrow(auxmetadata), '-D', 
                         (if(npoints==1 && all(is.na(data))){0}else{npoints}),
  ...
  if (missing(outputdir) && is.character(datafile)) {
    outputdir <- paste0('_output/', datafile)
    outputdir <- paste0(sub('.csv$', '', outputdir))
  } else if (missing(outputdir)) {
    outputdir <- '_output'
  }
  # The name of the output folder is on the form
  # <outputdir>/V<n_variates>-D<n_datapoints>-K<n_clusters>-S<n_samples>
  # If there is just one row and all data is missing, set <n_variates> to 0
  ndatapoints <- (if (npoints == 1 && all(is.na(data))) {
    0
  } else {
    npoints
  })
  dirname <- paste0(
    outputdir, '/V', nrow(auxmetadata), '-D', ndatapoints,
    '-K', nclusters, '-S', nsamples, '/'
  )
  ...
  chaininfo <- foreach::foreach(
    ...
    for (achain in 1:nchainspercore) {
      ...
      if (is.numeric(loglikelihood)) {
        llseq <- sort(sample(data_no_na, loglikelihood))
      }
      gc() #garbage collection
      ...
      while (nitertot < lengthmeasure) {
        ...
        ##                 to_remove <- which(!is.finite(mcsamples), arr.ind=TRUE)
        ##                 ##
        ...
        ##                     flagmc <- TRUE
        ##                     allflagmc <- TRUE
        ##                     if(length(unique(to_remove[,1])) == nrow(mcsamples)){
        ...
        ##                     }else{
        ##                         mcsamples <- mcsamples[-unique(to_remove[,1]),,drop=FALSE]
        ##                     }
        ##                 }
        #### Remove iterations with non-finite values
        to_remove <- sort(unique(unlist(lapply(mcsamples, function(xx) {
          temp <- which(is.na(xx), arr.ind = TRUE)
          temp[, ncol(temp)]
        }))))
        if (length(to_remove) > 0) {
          cat('\nWARNING:', length(to_remove), 'NON-FINITE SAMPLES\n')
          ##
          flagmc <- TRUE
          allflagmc <- TRUE
          ...
          if (length(to_remove) == ncol(mcsamples$W)) {
            ...
            mcsamples <- mcsubset(mcsamples, -to_remove)
          }
        }
        ...
        ## }
        ## dev.off()
        to_remove <- which(!is.finite(traces), arr.ind = TRUE)
        if (length(to_remove) > 0) {
          flagll <- TRUE
          traces <- traces[-unique(to_remove[, 1]), , drop = FALSE]
        }
        ##
