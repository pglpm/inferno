...
function to display values and quantiles together:
...
can produce it with the [`print()`](https://rdrr.io/r/base/print.html)
function as usual:
...
[`learn()`](https://pglpm.github.io/prova/reference/learn.md) function
(and as usual please wait before running the code below); this time we
use 8 parallel cores:
...
[^2]: The Monte Carlo expert will notice that the effective sample size
    is around 3000 or more. The Monte Carlo sampling is designed to stop
    when 3600 samples have been obtained from 8 chains, each with an
    effective sample size of at least 450, or equivalently a relative
    Monte Carlo standard error under 0.047. This is enough, considering
    that the statistical uncertainty in the results can be tens or
    hundreds of times larger than this. The desired number of samples
    and maximal Monte Carlo standard error can be specified with the
    `nsamples`, `minESS`, and `maxrelMCSE` arguments.
