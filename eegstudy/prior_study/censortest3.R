...
set.seed(229)
rdata <- cbind(
dt2$AVDEL30MIN_neuro[1:ndata]-1,
dt2$CATANIMSC_neuro[1:ndata]-1,
dt2$GDTOTAL_gds[1:ndata]-1,
dt2$ANARTERR_neuro[1:ndata]-1,
dt2$RAVLT_immediate[1:ndata]-1,
...
infmixture <- nimbleCode({
    ...
    for(datum in 1:ndata){
        ...
        for(variate in 1:ncens){# real variates
            Rdata[datum, variate] ~ dcat(prob=hidden[datum, variate])
...
## ncens=5, hidden:
## Time difference of 1.57342 mins
## Rvariances     Rmeans     Labels     hidden          q     Rrates     alpha0 
##  37.265518  32.122508  20.530362   2.379558   0.287333   0.241739   0.014687 
