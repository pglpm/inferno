...
We receive the first 10 datapoints. Here they are:

```{r echo = FALSE}
datafile <- 'penguindata2007_10.csv'
dat <- read.csv(datafile, na.strings = '', stringsAsFactors = FALSE, tryLogical = FALSE)

knitr::kable(dat[,-ncol(dat)], row.names = TRUE)
```

And with these datapoints we start our Bayesian nonparametric analysis using ***inferno***! There are two preliminary steps that we usually must follow to perform an analysis:
...
Metadata are "data about the data". They consist of around eight pieces of information about the variates of our population. Specifically:

Variate name (`name`)
: This is a character or string: name of a variate. Different variates should obviously have different names. It's advisable to choose names conforming to [R's rules](https://cran.r-project.org/doc/FAQ/R-FAQ.html#What-are-valid-names_003f).
