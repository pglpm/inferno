...
Upon learning that your penguin is from *Biscoe*, your degree of belief would change: the most probable value would be 4.8 kg at 1.3% probability. Note how the histograms for `body_mass` are different in the subpopulations of the three islands.
...
## Uncertainty about whole populations, and population samples
...
Therefore, to guess the frequencies in the whole population we can examine a sample from it, but we cannot rely fully on the sample.
...
It seems quite unnatural based on our experience with histograms over large natural populations. If a sample from a penguin population showed this frequency distribution of `body_mass` values, you would probably check if any errors were made during the sampling.

The calculation of such probabilities and of the most probable frequency distributions for the whole population is the core of the ***inferno*** package, realized by the `learn()` function.

## Bayesian nonparametric population inference

The traditional way of facing
