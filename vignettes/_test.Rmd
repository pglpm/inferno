---
title: "Probabilistic-statistical analysis with Prova"
output:
  rmarkdown::html_vignette:
    number_sections: true
vignette: >
  %\VignetteIndexEntry{Probabilistic-statistical analysis with Prova}
...
In our example we shall use data from the `datasets::penguins` dataset, included in R version 4.5.0 and above. We shuffle this dataset to erase any particular ordering of its data, and call the new dataset `penguin` (without the final 's'). For your convenience you can download the shuffled dataset as the CSV file [`penguin_data.csv`](https://github.com/pglpm/prova/raw/main/development/downloads/penguin_data.csv). Otherwise you can create such file from R 4.5.0 or above as follows:
...
The utility function `write.csvi()` saves the dataset as a CSV file that respects the [formatting rules required by **Prova**](#format).
...
```{r, echo = FALSE}
dat <- read.csvi('penguin_metadata_temp.csv')
...
The new frequency estimates for `Adelies` (solid red line) and `Chinstrap` (dashed blue line) are now around `r printprob(abs(diff(onlyDream60$values[c('Adelie','Chinstrap'),1])),1)` apart; the previous estimates had them around `r printprob(abs(diff(onlyDream10$values[c('Adelie','Chinstrap'),1])),1)` apart. This change is not unexpected: the new estimates are well within the previous credibility intervals; we were correctly warned about their possible change.
...
The new frequency estimates for `Adelies` (solid red line) and `Chinstrap` (dashed blue line) are now around `r printprob(abs(diff(onlyBiscoe60$values[c('Adelie','Chinstrap'),1])),1)` apart, whereas the previous estimates were almost equal. Again this is not unexpected, and these changes are well within the previous credibility intervals.
