...
This vignette gives an introduction and guide to the visual and quantitative capabilities and functions of the **Prova** package, for the study of associations, correlations, and links.

See the `vignette('start')` for an introduction to the package. The present vignette continues with the same example and dataset.

\

# Probabilities and associations

The `vignette('start')` focuses on the `datasets::penguins` dataset, and shows how to calculate probabilities of single and joint variates from that dataset; also probabilities conditional on given variates. These probabilities refers to a *new unit*; for example:
...
We shall now explore first some visual ways to examine associations with **Prova**, and then some more precise quantitative ways, which also apply when visualization becomes impossible.
...
Let's load the **Prova** package, if we haven't already done so, and set a random seed to ensure reproducibility:
...
As in the `vignette('start')`, we work with a specific population of penguins, of which we have 344 sample data stored in the [`datasets::penguins`] dataset, included in R version 4.5.0 and above. For your convenience you can download the shuffled dataset as the CSV file [`penguin_data.csv`](https://github.com/pglpm/prova/raw/main/vignettes/penguin_data.csv), then load it with the `pread.csv()` function as follows:

```{r loadpenguin, echo = FALSE}
datafile <- 'penguin_data.csv'
penguin <- pread.csv(datafile)
```

We assume that we have already learned all predictive information from this dataset by means of the **Prova** function `learn()`. This information is stored in a "learnt" object, which is either loaded in memory, or stored in the compressed file `learnt.rds` within the output directory that was specified in the `learn()` function. For your convenience the object produced by the computation mentioned above can be downloaded as the file [`learntall.rds`](https://github.com/pglpm/prova/raw/main/vignettes/learntall.rds). Once you have downloaded it in your working directory you can just set

```{r loadlearnt, eval = TRUE, echo = TRUE}
learnt <- 'learntall.rds'
```
in this case `learnt` is just a character string pointing to the file containing the actual "learnt" object.

## Generating new samples {#newsamples}

Let's focus on the `island` and `species` variates. Each variate has three possible values; **Prova**'s utility function `vrtgrid()` allows us to create a vector of all possible values of each variate, which are stored in the metadata of the `learnt` object:

```{r valuessample, eval = TRUE}
...
```{r vissample, eval = TRUE, fig.cap='**Scatter plot of sample data**'}
...
```{r probTorgersenChinstrap, eval = TRUE}
...
The **Prova** package offers the function `rPr()` to generate a data.frame of fictitious samples from the estimated frequencies of the whole population. These samples can be used for different purposes, for instance to produce a scatter plot. The main arguments of `rPr()` are:
...
```{r gensamplesplot, eval = TRUE, fig.cap='**Scatter plot for *whole* population**'}
...
We generate 2000 whole-population samples of the two variates with `rPr()`, and then scatter-plot them with `flexiplot()`. First we select an appropriate plot range for the continuous variate `body_mass` by means of **Prova**'s utility function `vrtgrid()`: this function chooses an optimal value based on, and including, the range of data previously observed:

```{r gensamplesplotbodymassspecies, eval = TRUE, fig.cap='**Scatter plot for body mass and species**'}
...
From the plot we see that there is an association between the Adélie and Chinstrap species and lighter body mass, around 3500 g; and between the Gentoo species and heavier body mass, around 5000 g. Incidentally, this reflects the probability plot at the end of the `vignette('start')`.
...
```{r gensamplesplotbodybilllen, eval = TRUE, fig.cap='**Scatter plot of body mass and bill length**'}
...
```{r gensamplesbodybilllen3, eval = TRUE}
samplesAdelie <- rPr(
    n = 1000,
...
```{r plotbodybilllen3, eval = TRUE, fig.cap='**Scatter plot for species subpopulations**'}
flexiplot(
...
```{r pearsonplot, eval = TRUE, fig.cap='**Perfect correlation from $Y_1$ to $Y_2$, with zero Pearson correlation coefficient**'}
...
\
The **Prova** package provides the function `mutualinfo()` to calculate the mutual information between two variates or two sets of joint variates. Its main arguments are the following:
...
```{r loadpmiis, echo = FALSE}
MIislandspecies <- readRDS('MIislandspecies.rds')
MIbodymassspecies <- readRDS('MIbodymassspecies.rds')
MIbodymassbilllen <- readRDS('MIbodymassbilllen.rds')

MIadelie <- readRDS('MIadelie.rds')
MIchinstrap <- readRDS('MIchinstrap.rds')
MIgentoo <- readRDS('MIgentoo.rds')
```
...
```{r miislandspeciesMI, eval = TRUE}
MIislandspecies$MI
```

Between variates `island` and `species` there is thus a mutual information of `r signif(MIislandspecies$MI['value'], 2)` Sh. But what does this mean?
...
```{r Ivsr, eval = TRUE, echo = FALSE, fig.cap='**$I$ vs $\\lvert r\\rvert$ for jointly Gaussian variates**', out.width = '75%'}
x <- seq(0, 1, length.out = 129)
y <- -0.5 * log2(1-x^2)
flexiplot(x = x, y = y,
    xlab = bquote('Pearson'~group('|', italic(r), '|')),
    ylab = bquote('mutual information'~italic(I))
    )
```
...
```{r miislandspeciesMIr, eval = TRUE}
MIislandspecies$MI
...
```{r gensamplesplotbis, eval = TRUE, fig.cap='**Scatter plot for `island` and `species`**'}
...
```{r mibodymassspecies, eval = FALSE}
...
```{r gensamplesplotbodymassspeciesbis, eval = TRUE, fig.cap='**Scatter plot and mutual info for body mass and species**'}
## mutual info
...
```{r mibodymassbilllen, eval = FALSE}
...
```{r gensamplesplotbodymassbilllenbis, eval = TRUE, fig.cap='**Scatter plot and mutual info for body mass and bill length**'}
## mutual info
...
```{r pearsonbmbl, eval = TRUE}
cor(samples$body_mass, samples$bill_len, method = 'pearson')
```

which is different from the rough $r$-equivalent `r r`.
...
```{r plotbodybilllen3mi, eval = TRUE, fig.cap='**Scatter plot and mutual info for species subpopulations**'}
## mutual info, joined
mispecies <- signif(c(
    MIadelie$MI['value'],
...
*(To be continued in future versions)*
-->
...
- Cover, Thomas: [*Elements of Information Theory*](https://doi.org/10.1002/0471200611) (2nd ed. 2005).

- MacKay: [*Information Theory, Inference, and Learning Algorithms*](https://www.inference.org.uk/itila/book.html) (2005).

- Chapter "Information, relevance, independence, association" in [*Data Science and AI Prototyping*](https://pglpm.github.io/ADA511/information.html).
