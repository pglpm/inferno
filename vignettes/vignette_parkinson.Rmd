...
```{r}
flexiplot(x = (-30):30, y = probabilities$samples[,1,],
    ylim = c(0, max(probabilities$quantiles)), # keep same y-range as before
    xlab = 'MDS-UPDRS-III difference', ylab = 'probability',
    col = adjustcolor(3,0.25), lty = 1)

flexiplot(x = (-30):30, y = probabilities$values[,1], col = 3, lwd = 2,
    add = TRUE)
