...
probability0$value
```
that is, there is approximately a `r signif(probability0$value *100, 1)`% probability that the next treated female patient will have a MDS-UPDRS-III difference of exactly 0.
...
```
that is, if we had many more clinical trials, the probability above could change to a value within around `r signif(probability0$quantiles[,,1] * 100, 1)`% and `r signif(probability0$quantiles[,,2] * 100, 1)`%.
...
tplot(x = (-30):30, y = probabilities$values[,1], add = TRUE)
```
...
probDecrease$value
```
or approximately `r signif(probDecrease$value * 100, 1)`%; note that it is clearly higher than 50%.

Further clinical trials, however could update this probability to a new value between `r signif(probDecrease$quantiles[1,1,1] * 100, 1)`% and `r signif(probDecrease$quantiles[1,1,2] * 100, 1)`%:
```{r}
probDecrease$quantiles[1,1,]
```

```{r}
hist(probDecrease$samples[1,1,], plot = TRUE)
```
