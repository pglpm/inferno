...
probability$value
```
that is, there is approximately a `r signif(probability$value *100, 1)`% probability that the next treated female patient will have a MDS-UPDRS-III difference of exactly 0.
...
```
that is, if we had many more clinical trials, the probability above could change to a value within around `r signif(probability$quantiles[,,1] * 100, 1)`% and `r signif(probability$quantiles[,,2] * 100, 1)`%.
