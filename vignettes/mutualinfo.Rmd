...
- If we find that $\operatorname{Pr}(\text{species}\mathrel{\!=\!} \text{Adelie}) = 44\%$, then there's a 44% probability that the next penguin we sample (from the same population) is of the *Adélie* species.
- If we find that $\operatorname{Pr}(\text{species}\mathrel{\!=\!} \text{Gentoo} \,\vert\, \text{island} \mathrel{\!=\!} \text{Biscoe}) = 1\%$, then if we have a new penguin, and we know it is from Biscoe island, there's a 1% probability that it is of the *Gentoo* species.
...
# Drawing and plotting *new* samples
...
The ***inferno*** package offers the function `rPr()` to generate fictitious samples from the estimated frequencies of the whole population. These samples are collected in a data frame. They samples can be used for different purposes, for instance to produce a scatter plot. The main arguments of `rPr()` are:
...
#           island   species
# 255_1  Torgersen    Adelie
# 826_1      Dream Chinstrap
# 1418_1 Torgersen    Adelie
# 2394_1     Dream    Adelie
# 2855_1 Torgersen    Adelie
```
