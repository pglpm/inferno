...
## Last-Updated: 2022-01-06T10:20:13+0100
################
    ...
    for(acluster in 1:nClusters){
        if(nrcovs>0){# real variates
            for(avar in 1:nRcovs){
                meanR[avar,acluster] ~ dnorm(mean=meanRmean0[avar], tau=meanRtau[avar,acluster])
                meanRtau[avar,acluster] ~ dgamma(shape=meanRshape0[avar], rate=meanRrate0[avar])
                tauR[avar,acluster] ~ dgamma(shape=tauRshape0[avar], rate=tauRrate[avar,acluster])
                tauRrate[avar,acluster] ~ dgamma(shape=tauRshape0[avar], rate=tauRrate0[avar])
            }
        }
        if(nicovs>0){# integer variates
...
if(posterior){
    model <- nimbleModel(code=bayesnet, name='model1', constants=constants, inits=initsFunction(), data=dat, dimensions=list(q=nclusters, meanRtau=c(nrcovs,nclusters), tauRrate=c(nrcovs,nclusters), meanR=c(nrcovs,nclusters), tauR=c(nrcovs,nclusters), probI=c(nicovs,nclusters), probB=c(nbcovs,nclusters), C=ndata) )
}else{
    model <- nimbleModel(code=bayesnet, name='model1', constants=constants, inits=initsFunction(), data=list(), dimensions=list(q=nclusters, meanRtau=c(nrcovs,nclusters), tauRrate=c(nrcovs,nclusters), meanR=c(nrcovs,nclusters), tauR=c(nrcovs,nclusters), probI=c(nicovs,nclusters), probB=c(nbcovs,nclusters)))
}
    ...
    for(acluster in 1:nclusters){
        if(nrcovs>0){
            for(avar in 1:nrcovs){
                ...
                confmodel$addSampler(target=paste0('meanRtau[', avar, ', ', acluster, ']'), type='conjugate')
                confmodel$addSampler(target=paste0('tauR[', avar, ', ', acluster, ']'), type='conjugate')
                confmodel$addSampler(target=paste0('tauRrate[', avar, ', ', acluster, ']'), type='conjugate')
            }
        }
        if(nicovs>0){
            for(avar in 1:nicovs){
