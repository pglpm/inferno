...
## Last-Updated: 2022-04-27T09:22:34+0200
################
...
if(nrcovs>0){
    ...
    widthrcovs <- apply(alldata[1:ndata,..realCovs],2,function(x)IQR(x, na.rm=TRUE, type=8)/(2*qnorm(3/4)))
}
##
...
## checkprobsFile <- paste0(dirname,'/_checkprobs-R',baseversion,'_',mcmcseed,'_','-V',length(covNames),'-D',ndata,'-K',nclusters,'.rds')
...
for(stage in stagestart+(0:nstages)){
    ...
    if(stage==stagestart){
        if(exists('continue') && is.character(continue)){
            ...
            mcsamples <- runMCMC(Cmcmcsampler, nburnin=0, niter=niter*thin, thin=thin, thin2=niter*thin, inits=inits0, setSeed=mcmcseed)
        }else{
            inits0 <- initsFunction
...
ptraces <- foreach(i=1:128, .combine=rbind)%dopar%{readRDS(paste0(dirname,"/_probtraces-R_presults1_",i,"_0-V3-D8192-K64-I2048.rds"))}
test <- colnames(ptraces)
dim(ptraces) <- c(2048,128,65)
dimnames(ptraces) <- list(NULL,NULL,test)

llt <- c(readRDS("_results5_111_-V3-D8192-K64-I1024/_probtraces-R_results5_111_2-V3-D8192-K64-I1024.rds")[,1],
         readRDS("_results5_111_-V3-D8192-K64-I1024/_probtraces-R_results5_111_2-V3-D8192-K64-I1024.rds")[,1])

lll <- ptraces[,,1]
lld <- apply(lll,2,function(x){diff(x,lag=2)/2})
##lld <- apply(lll,2,function(x){diff(x,lag=1)/2})

mult <- 16
epoint <- 2048
lpoint <- epoint+2048
tplot(x=epoint+(1:2048), y=llt, col=3, lty=1, lwd=2, ##alpha=0.9,
                xlim=c(1,lpoint), ylim=c(-20200,max(lll[1:epoint,],llt)))
for(i in 1:128){
    tpoint <- (which(lld[,i]<0)[1])*mult
    ## tplot(x=1:(tpoint-1), y=lll[1:(tpoint-1),i], col=2, lty=1, lwd=2, alpha=0.9,
    ##       # xlim=c(1,epoint), ylim=c(-20400,max(lll[1:epoint,])),
    ##       add=T)
    tplot(x=tpoint:epoint, y=lll[tpoint:epoint,i], col=1, lty=1, lwd=2, alpha=0.9,
          add=T)
}
