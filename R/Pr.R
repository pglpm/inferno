...
#' @param tails named vector or list, or `NULL` (default). The names must match some or all of the variates in arguments `Y` and `X`. For variates in this list, the probability arguments are understood in an semi-open interval sense: $Y \le y$ or $Y \ge y$, an so on. This is true for variates on the left and on the right of the conditional sign "$\|$". A left-open interval "$Y \le y$" is indicated by the values `'<='` or `'left'` or `-1`; a right-open interval "$Y \ge y$" is indicated by the values `'>='` or `'right'` or `+1`. Values `NULL`, `'=='`, `0` indicate that a point value "$Y = y$" (not an interval) should be calculated. NB: the semi-open intervals *always* include the given value; this is important for ordinal or rounded variates. Therefore, if `Y` is for instance an integer variate, then to calculate  `P(Y < 3)` you should require `P(Y <= 2)`.
