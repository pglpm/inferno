...
#'
#' @return A list of class `probability`, identical to the original object `p` except for a reduced range of values in some if its variates.
#'
#' @export
subset.probability <- function(
    ...
    for(vrt in vrtnames[vrtnames %in% Ynames]){
        selvals <- p$Y[[vrt]] %in% subset[[vrt]]
        x$values <- p$values[selvals, , drop = FALSE]
        if(!is.null(p$quantiles)){
            p$quantiles <- p$quantiles[selvals, , , drop = FALSE]
        }
        if(!is.null(p$samples)){
            p$samples <- p$samples[selvals, , , drop = FALSE]
        }
        p$Y <- p$Y[selvals, , drop = FALSE]
    }

    ## subset X
    for(vrt in vrtnames[vrtnames %in% Xnames]){
        selvals <- p$X[[vrt]] %in% subset[[vrt]]
        p$values <- p$values[, selvals, drop = FALSE]
        if(!is.null(p$quantiles)){
            p$quantiles <- p$quantiles[, selvals, , drop = FALSE]
        }
        if(!is.null(p$samples)){
