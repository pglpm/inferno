...
datapoints <- c(
    list(Rdata = rdata, Censored=censored)
)
##
constants <- list(ndata=ndata, nclusters=nclusters, mbounds=mbounds, nvars=nvars, ncens=ncens, maxbounds=maxbounds)
##
...
infmixture <- nimbleCode({
    ...
    for(datum in 1:ndata){
        ...
        for(variate in 1:ncens){# real variates
            Censored[datum, variate] ~ dinterval(Rdata[datum, variate], mbounds[variate,maxbounds])
        }
        for(variate in 1:nvars){
